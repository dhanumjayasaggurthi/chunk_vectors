...
class AzureGateway:
    ...
    def _url(config: EndpointConfig, path: str) -> str:
        return f"{config.endpoint}/openai/deployments/{config.deployment}/{path}?api-version={config.api_version}"
    ...
    def _image_mime(image_bytes: bytes) -> str:
        ...
        if image_bytes.startswith((b"GIF87a", b"GIF89a")):
            return "image/gif"
        # PDF page rendering is PNG; unknown embedded formats are deliberately
        # rejected instead of being mislabeled in a multimodal request.
    ...
    def embeddings(self, texts: list[str]) -> list[list[float]]:
        config = self._section(("AZURE_OPENAI_EMBEDDING", "azure_embedding"), required_model="text-embedding-3-large", required_api_version="2025-04-01-preview")
        data = self._post(self._url(config, "embeddings"), config.api_key, json_body={"model": config.model, "input": texts}).json()
        items = sorted(data.get("data", []), key=lambda item: item.get("index", 0))
        vectors = [item.get("embedding") for item in items]
        if len(vectors) != len(texts) or any(not isinstance(vector, list) or len(vector) != 3072 for vector in vectors):
    ...
    def chat_json(self, messages: list[dict[str, Any]], *, max_tokens: int = 3000, temperature: float = 0.0) -> dict[str, Any]:
        ...
        payload = {"model": config.model, "messages": messages, "temperature": temperature, "max_tokens": max_tokens, "response_format": {"type": "json_object"}}
        try:
            data = self._post(self._url(config, "chat/completions"), config.api_key, json_body=payload).json()
    ...
    def describe_chart(self, image_bytes: bytes, context: str = "") -> str:
        prompt = (
            "Extract only information visibly supported by this chart/graph. Report chart type, visible title, axes/units, legend labels, explicit plotted values when readable, and clearly visible trends. If something is not readable, say it is not readable; do not infer values. "
    ...
    def describe_image(self, image_bytes: bytes, context: str = "") -> str:
        prompt = (
            "Describe only content visibly supported by this document image for retrieval. Include visible subject matter, labels, annotations, captions, symbols, and scientifically relevant relationships that can be directly observed. Do not infer identity, measurements, diagnoses, values, or conclusions that are not visibly supported. If the image is decorative or contains no meaningful retrievable content, say so briefly. "
            + (f"Nearby document text: {context[:1000]}" if context else "")
        )
